<?php
    $id = $_GET['ebook_by_cat'];

    $query1 = "SELECT * FROM tbl_course JOIN tbl_author ON tbl_course.author = tbl_author.id_author JOIN tbl_category ON tbl_course.category = tbl_category.cat_id WHERE tbl_course.status = 1 AND tbl_course.category = '".$id."' ORDER BY tbl_course.views DESC";
